...
namespace Incapital.BizTalk.PipelineComponents
{
    public class PGPWrapper
    {
        ...
        public static Stream DecryptStream(Stream inStream, string privKeyFile, string passphrase, string encryptedFilename, string tempDir)
        {
            ...
            string tmpDecryptedFile = string.Empty;
            if (encryptedFilename.LastIndexOf(".") > 0)
            {
                tmpDecryptedFile = encryptedFilename.Substring(0, encryptedFilename.LastIndexOf("."));
            }
            else
            {
                tmpDecryptedFile = encryptedFilename;
            }
